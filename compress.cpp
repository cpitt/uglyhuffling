...
class Huffman{
  private:
     ...
     * and for our key. I found that using simple arrays was
     * much faster than unordered hash.....Need to test tree traversal
     * **************************************************************/
  ...
  public:
     ...
     * **********************************/
    vector<unsigned char> generate_bytes(string binary_string){
    ...
    string encode(string &to_encode){
      this->binary_string_length = to_encode.length();
      this->build_frequency_table(to_encode);
      this->build_huffman_tree();
      this->generate_key(this->root, "");
      string encoded_binary_string = to_binary_string(to_encode);
      vector<unsigned char> bytes = generate_bytes(encoded_binary_string);
      this->save_zip301(bytes, "test.zip301");
    }
};
